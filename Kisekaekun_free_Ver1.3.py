...
- 2026-06-07: Kisekaekun_free_Ver1.3 
              ①実際の写真での白飛び不具合を解消するため、手動で影のバランスを調整できる「白黒の境界値スライダー」を新設。
              ②スマホ実機テスト時のプレビュー画像潰れ、および画像未選択時のNameError、デプロイ時のKeyErrorを完全にクリア。
              ③写真本来のアスペクト比を100%維持して最上部に美しく表示する仕様へブラッシュアップ。
              ④不要な過去の試行錯誤の履歴（Ver1.1/Ver1.2）をルールに基づき破棄し、Ver1.3として正式リリース。
              出力エクセル名も「Compass_free_Ver1.3.xlsx」に連動更新。
"""

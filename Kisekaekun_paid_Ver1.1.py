...
バージョン: Kisekaekun_paid_Ver1.1
修正履歴:
- 2026-06-06: Kisekaekun_paid_Ver1.0 初版リリース。パスワード認証機能を搭載。
- 2026-06-06: Kisekaekun_paid_Ver1.1 鈴木英人風アートへの進化。
              ①ドロドロ化を防ぐ「平均値シフトフィルタ(MeanShift)」によるイラスト調スムージングに刷新。
              ②輪郭線をパキッと目立たせる「Cannyエッジ＋拡張処理」によるライン太さ調整スライダーを新設。
              ③5レイヤー境界設定のラベルを「領域①」の形へ究極にシンプル化（settings.json連動）。
              連動するエクセル名も「Compass_paid_Ver1.1.xlsx」へアップデート。
"""
...
st.set_page_config(page_title="KAKERA きせかえくん paid_Ver1.1", layout="wide")
...
st.title(f"🎨 {T['title']} - Paid Ver1.1")
...
# イラスト調フィルター（空間窓・色空間窓の強さを30段階でマッピング）
smooth_strength = st.sidebar.slider(T["slider_smooth"], 0, 30, 0, key=f"smooth_{r_idx}")
# 【Ver1.1新設】鈴木英人風の黒ラインの太さ調整スライダー
line_thickness = st.sidebar.slider(T["slider_line"], 0, 5, 0, key=f"line_{r_idx}")
...
if uploaded_master is not None:
    ...
    # 【Ver1.1ロジック①】平均値シフトフィルタによるツルッとしたイラスト調前処理
    ...
    # 【Ver1.1ロジック②】鈴木英人風のアートライン（輪郭線）をブレンド
    if line_thickness > 0:
        # Cannyエッジ検出で色の境界線を抽出
        edges = cv2.Canny(gray, 30, 100)
        # スライダーに合わせて線の太さを太らせる（モルフォロジー膨張）
        kernel = np.ones((line_thickness, line_thickness), np.uint8)
        dilated_edges = cv2.dilate(edges, kernel, iterations=1)
        # 輪郭部分を黒(0.0)にするためのマスクを作成
        line_mask = (dilated_edges == 0).astype(float)[:, :, np.newaxis]
        result_img = result_img * line_mask
        
    preview_img = cv2.cvtColor(result_img.clip(0, 255).astype(np.uint8), cv2.COLOR_BGR2RGB)
    
    # バージョン連動対象ファイル名
    filename_ver = "Compass_paid_Ver1.1.xlsx"

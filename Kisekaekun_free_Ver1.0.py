...
# 画面全体の初期設定
st.set_page_config(page_title="KAKERA きせかえくん free_Ver1.0", layout="wide")

# --- 🌐 言語切り替え設定 ---
# 無料版は誰もが直感的に使えるよう、サイドバーではなく最上部（スマホでも見やすい位置）に配置
selected_lang = st.selectbox("🌐 Language / 言語切り替え", list(LANG_DICT.keys()))
T = LANG_DICT[selected_lang]

st.title(f"🎨 {T['title']} - Free Ver1.0")
...
# --- 🛠️ レイアウトの出し分け（ここがドンピシャ仕様！） ---

# 【パターンA：スマホ版】サイドバーを使わず、上から下へ流れる1カラムで親指操作！
if is_mobile:
    ...
    display_width = 320 # スマホ画面にピタッと収まるサイズ

# 【パターンB：PC・タブレット版】左に操作パネル、右に大迫力プレビューの見開き2カラム！
else:
...
if uploaded_master is not None:
    ...
    # 【無料版2領域化】大津の二値化（Otsu's Thresholding）を使い、画像に応じて最適な中間値で白黒に一発自動分割！

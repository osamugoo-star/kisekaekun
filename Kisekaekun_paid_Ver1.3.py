...
- 2026-06-06: Kisekaekun_paid_Ver1.3 輪郭ラインの細密化（10分の1微調整）仕様追加。
              ①スライダーの 0〜10 の値を、内部で 10分の1（0.0〜1.0）の係数としてブレンド処理。
              ②これにより、Ver1.2の太さ1の「10分の1の細さ（半透明の繊細なニュアンス）」を「1」として指定可能に。
              連動するエクセル名も「Compass_paid_Ver1.3.xlsx」へ完全連動。
"""
...
st.set_page_config(page_title="KAKERA きせかえくん paid_Ver1.3", layout="wide")
...
st.title(f"🎨 {T['title']} - Paid Ver1.3")
...
# 【Ver1.3細密化】0〜10の整数で指定してもらい、内部で10分の1にする
...
if uploaded_master is not None:
    ...
    # 【Ver1.3新ロジック】すべての領域の「境界のキワ」に、10分の1ずつの滑らかな濃さでラインをブレンド
    if line_input_val > 0:
        # 10分の1のブレンド係数（0.1 〜 1.0）を計算
        ...
        # 境界線マスクをfloat化（線がある場所が1.0、ない場所が0.0）
        edge_mask = (combined_edges > 0).astype(float)[:, :, np.newaxis]
        
        # 線がある場所に対して、指定された濃さ(line_weight)の分だけ黒(0,0,0)をアルファブレンドする
        # line_weightが 0.1 ならほんのり薄い線、1.0 ならパキッと完全な黒い線になる
    ...
    # バージョン連動
    filename_ver = "Compass_paid_Ver1.3.xlsx"
